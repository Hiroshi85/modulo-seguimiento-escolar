        ...
        <div class="mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-5">
                <div class="px-6 py-4 text-gray-900 dark:text-gray-100 flex justify-between">
                    <p class="font-xl text-gray-800 dark:text-white font-semibold py-2 px-4">Editar apoderado</p>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('apoderados.update', $apoderado->id) }}" class="max-w-7xl mx-auto">
                        @method('put')
                        @csrf
                        <div class="grid grid-cols-2 gap-4 lg:grid-cols-3">
                          <div>
                            <label for="nombres" class="block">Nombres:</label>
                            <input required type="text" id="nombres" name="nombres" class="w-full" value="{{$apoderado->nombres}}">
                          </div>
                          <div>
                            <label for="apellidos" class="block">Apellidos:</label>
                            <input required type="text" id="apellidos" name="apellidos" class="w-full" value="{{$apoderado->apellidos}}">
                          </div>
                          <div>
                            <label for="telefono" class="block">Teléfono:</label>
                            <input required type="text" id="telefono" name="telefono" class="w-full" value="{{$apoderado->telefono}}">
                          </div>
                          <div>
                            <label for="correo" class="block">Correo:</label>
                            <input required type="email" id="correo" name="correo" class="w-full" value="{{$apoderado->correo}}">
                          </div>
                          <div>
                            <label for="edad" class="block">Edad:</label>
                            <input required type="number" id="edad" name="edad" class="w-full" max="100" min="18" value="{{$apoderado->edad}}">
                          </div>
